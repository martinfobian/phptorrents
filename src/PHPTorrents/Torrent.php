...
class Torrent extends TorrentController
{
    ...
    public function setSize($bytes)
	{
        ...
        else
        {
            throw new \InvalidArgumentException(sprintf(
                '"%s": Invalid torrent size provided. Expected int > -1, "%s" given',
                'setSize', $bytes
            ));
        }
	}
	...
	public function setDownloadSpeed($bytesPerSecond)
	{
		...
		{
			throw new \InvalidArgumentException(sprintf(
				'"%s": Invalid count of bytes provided. Should be > -1, given %s',
	...
	public function setBytesDownloaded($bytes)
	{
		if(is_int($bytes) && $bytes > -1)
		{
			...
			$this->ETA = (int) number_format(
				($this->size / (
					0 == $this->downloadSpeed ? 1 : $this->downloadSpeed
				)) * 100,
				2, '.', ''
			);
			return $this;
