...
abstract class ClientFactory extends ClientController
{
    ...
    public function _deleteTorrent(Torrent $torrent, $removeData = false)
    {
        if (!is_null($torrent))
        {
            $arguments = array($torrent->getHashString(), $removeData);
        ...
        {
            throw new \InvalidArgumentException(sprintf('"%s": Valid Torrent instance expected, "%s" given',
    ...
    public function _addTorrent(Torrent $torrent = null)
    {
        if (!is_null($torrent) && method_exist($torrent, 'getAddParams'))
        {
            ...
            $method = $parameters['type'] == 'magnet' ? self::METHOD_ADD_MAGNET : self::
                METHOD_ADD_TORRENT;
    ...
    public function _startTorrent(Torrent $torrent = null)
    {
        if (!is_null($torrent))
        {
            try
            {
                return $this->client->finalRequest(self::METHOD_START, array(array($torrent->
                            getHashString())));
            }
            catch (HTTPException $e)
            {
    ...
    public function _pauseTorrent(Torrent $torrent = null)
    {
        if (!is_null($torrent))
        {
            try
            {
                return $this->client->finalRequest(self::METHOD_PAUSE, array(array($torrent->
                            getHashString())));
            }
            catch (HTTPException $e)
            {
    ...
    public function _queueTorrent(Torrent $torrent = null, $target)
    {
        ...
        if (array_key_exists($target, $directionMap) && !is_null($torrent))
        {
            try
            {
                return $this->client->finalRequest($directionMap[$target], array(array($torrent->
                            getHashString())));
            }
            catch (HTTPException $e)
            {
        ...
        {
            throw new \InvalidArgumentException(sprintf('"%s": Direction expected, "%s%" given',
                'queueTorrent', $target));
        }
    }
    public function mapTorrent($torrent)
    {
        ...
        $object = $this->buildTorrent($torrent->hash, $this)->setName($torrent->name)->
            setSize($torrent->total_wanted)->setDownloadSpeed($torrent->
            download_payload_rate)->setUploadSpeed($torrent->upload_payload_rate)->
            setStatus($torrent->state)->setBytesUploaded($torrent->total_uploaded)->
