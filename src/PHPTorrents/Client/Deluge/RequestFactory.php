...
class RequestFactory
{
	...
	public function finalRequest($method, array $params = array())
	{
		...
		return $this->performRequest($method, $params, array(
			'Cookie'	=> array(
				$cookie
			)
		));
	...
	{
		$response = $this->performRequest(self::METHOD_AUTH, array(
			$this->connectionClient->getPassword()
		));
		...
		{
			throw new DelugeException(sprintf(
				'"%s": No cookie string received, authentication failed',
				self::METHOD_AUTH
			));
		}
	}
	private function performRequest($method, array $params = array(), array $headers = array())
	{
		...
		$request = (new \Amp\Artax\Request)
			->setUri(
				sprintf(
					self::TARGET_URI,
					$this->connectionClient->getProtocol(),
					$this->connectionClient->getHost(),
					$this->connectionClient->getPort()
				)
			)
		...
		if($response->getStatus() === 200)
		{
			...
			if(json_decode($body, true) !== null)
			{
                ...
                {
                    throw new DelugeException(sprintf(
                        '"%s": Error intercepted during request: #%s %s',
			...
			{
				throw new DelugeException(sprintf(
					'"%s": Invalid response received. Expecting JSON, "%s" given',
		...
		{
			throw new DelugeException(sprintf(
				'"%s": Incorrect HTTP status received. Expecting 200, "%s" given. Reason: "%s"',
